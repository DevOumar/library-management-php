...
if(isset($_GET['del']))
  {
    ...
    $query -> execute();
    $_SESSION['delmsg']="Emprunt supprimé avec succès ! ";
    header('location: gestion-emprunt-livre');
...
<div class="panel-body">
  <div class="info-box">
    <h4 class="text-black"><i class="fa fa-book"></i> Livres empruntés(<?php 
        $sql1 ="SELECT id from tblissuedbookdetails ";
        $query1 = $dbh -> prepare($sql1);
        $query1->execute();
        $results1=$query1->fetchAll(PDO::FETCH_OBJ);
        $issuedbooks=$query1->rowCount();
        ?><?php echo htmlentities($issuedbooks);?>) <a href="emprunt-livre" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i> Affecter un livre</a></h4>
    <hr />
  ...
  <table id="dataList" class="table table-bordered table-hover" data-name="cool-table">
    ...
    <tbody>
     <?php $sql = "SELECT tblstudents.FullName,tblissuedbookdetails.fine,tblstudents.Cycle,tblissuedbookdetails.Delai_livre,tblbooks.BookName,ranger.ranger,casier.casier,tblbooks.Nbre_page,tblbooks.ISBNNumber,tblissuedbookdetails.IssuesDate,tblissuedbookdetails.ReturnDate,tblissuedbookdetails.id as rid from  tblissuedbookdetails join tblstudents on tblstudents.StudentId=tblissuedbookdetails.StudentId join tblbooks on tblbooks.id=tblissuedbookdetails.BookId JOIN ranger ON ranger.id_ranger=tblbooks.Ranger 
        JOIN casier ON casier.id_casier=tblbooks.Casier order by tblissuedbookdetails.id desc";
$query = $dbh -> prepare($sql);
$query->execute();
  ...
  ?>                                     
        ...
        echo htmlentities("Retourné");
          ...
          <td class="center">
            ...
            <a class="delete" href="gestion-emprunt-livre?del=<?php echo htmlentities($result->rid);?>" data-id="<?php echo htmlentities($result->rid);?>"><i title="Supprimer" class="fa fa-trash"></i></a>
            </td>
          </tr>
          <?php $cnt=$cnt+1;}} ?>                                      
        </tbody>
...
$('body').on('click', '.delete', function (e) {
                ...
                swal({
                  ...
                  closeOnConfirm: false,
                    ...
                    $.ajax({
                        url: "gestion-emprunt-livre?del="+id,
                        cache: false,
                        async: true
                    })
