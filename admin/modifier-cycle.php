...
{   
  ...
  {
    $niveau=$_POST['niveau'];
    $cylid=intval($_GET['cylid']);
    $sql="UPDATE  cycle set Nom_cycle=:niveau where id=:cylid";
    $query = $dbh->prepare($sql);
    $query->bindParam(':niveau',$niveau,PDO::PARAM_STR);
    $query->bindParam(':cylid',$cylid,PDO::PARAM_STR);
    $query->execute();
    ...
    <div class="col-lg-12">
      <div class="card card-outline">
        <div class="card-header bg-primary">
          <h5 class="text-white m-b-0">Modifier Cycle</h5>
        </div>
        ...
        <div class="card-body">
          <form role="form" method="post">
            <?php 
            $cylid=intval($_GET['cylid']);
            $sql="SELECT * from cycle where id=:cylid";
            $query=$dbh->prepare($sql);
            $query-> bindParam(':cylid',$cylid, PDO::PARAM_STR);
            $query->execute();
            ...
            if($query->rowCount() > 0)
            {
              ...
              {               
                ...
                <div class="form-group">
                  ...
                  <input class="form-control" type="text" name="niveau" value="<?php echo htmlentities($result->Nom_cycle);?>" required />
                </div>
              <?php }} ?>
