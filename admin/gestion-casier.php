...
if(isset($_GET['accepte-cookie'])){
  ...
  header('Location: gestion-casier');
}
else{
  if(isset($_GET['del']))
  {
    $id_casier=$_GET['del'];
    $sql = "DELETE from casier  WHERE id_casier=:id_casier";
    $query = $dbh->prepare($sql);
    $query -> bindParam(':id_casier',$id_casier, PDO::PARAM_STR);
    $query -> execute();
    $_SESSION['delmsg']="Casier supprimé avec succès !";
    header('location:gestion-casier');
  ...
  <head>
    ...
    <title>BIBLIOTHEQUE | CYCLE </title>
...
<div class="panel-body">
  <div class="info-box">
    <h4 class="text-black"><i class="fa fa-spinner"></i> Casiers(<?php 
        $sql3 ="SELECT id_casier from casier ";
        $query3 = $dbh -> prepare($sql3);
        $query3->execute();
        $results3=$query3->fetchAll(PDO::FETCH_OBJ);
        $regstds=$query3->rowCount();
        ?><?php echo htmlentities($regstds);?>) <a href="ajouter-casier" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i> Nouveau</a></h4>
    <hr />
      ...
      if($query->rowCount() > 0)
      {
        ...
        $update_date=date('d-m-Y à H:i:s',$format_update_date); 
            ...
            <tr class="odd gradeX">
              ...
              <td><span class="label label-success"><?php echo htmlentities($result->casier);?></span></td>
              ...
              <td class="center">
                ...
                <a href="modifier-casier?csrid=<?php echo htmlentities($result->id_casier);?>"><i title="Modifier" class="fa fa-edit"></i></a>
                 <a class="delete" href="gestion-casier?del=<?php echo htmlentities($result->id_casier);?>" data-id="<?php echo htmlentities($result->id_casier);?>"><i title="Supprimer" class="fa fa-trash"></i></a>
                  </td>
                </tr>
                <?php $cnt=$cnt+1;}} ?>                                      
              </tbody>
