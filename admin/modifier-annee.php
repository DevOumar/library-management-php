...
{   
  ...
  if(isset($_POST['update']))
  {
    ...
    $anneeid=intval($_GET['anneeid']);
    $sql="UPDATE  annee set libelle=:annee_emprunt where id=:anneeid";
    $query = $dbh->prepare($sql);
    $query->bindParam(':annee_emprunt',$annee_emprunt,PDO::PARAM_STR);
    $query->bindParam(':anneeid',$anneeid,PDO::PARAM_STR);
    $query->execute();
    $_SESSION['updatemsg']="Année mise à jour avec succès !";
    ...
    <div class="col-lg-12">
      <div class="card card-outline">
        <div class="card-header bg-primary">
          <h5 class="text-white m-b-0">Modifier Année</h5>
        </div>
        ...
        <div class="card-body">
          <form role="form" method="post">
            <?php 
            $anneeid=intval($_GET['anneeid']);
            $sql="SELECT * from annee where id=:anneeid";
            $query=$dbh->prepare($sql);
            $query-> bindParam(':anneeid',$anneeid, PDO::PARAM_STR);
            $query->execute();
            ...
            if($query->rowCount() > 0)
            {
              ...
              {               
                ...
                <div class="form-group">
                  <label>Mois</label>
                  <input class="form-control" type="text" name="annee_emprunt" value="<?php echo htmlentities($result->libelle);?>" required />
                </div>
              <?php }} ?>
