...
if(isset($_GET['accepte-cookie'])){
  ...
  header('Location: gestion-ranger');
}
else{
  if(isset($_GET['del']))
  {
    $id_ranger=$_GET['del'];
    $sql = "DELETE from ranger  WHERE id_ranger=:id_ranger";
    $query = $dbh->prepare($sql);
    $query -> bindParam(':id_ranger',$id_ranger, PDO::PARAM_STR);
    $query -> execute();
    $_SESSION['delmsg']="Rangée supprimée avec succès !";
    header('location:gestion-ranger');
  ...
  <head>
    ...
    <title>BIBLIOTHEQUE | RANGEE </title>
...
<div class="panel-body">
  <div class="info-box">
    <h4 class="text-black"><i class="fa fa-spinner"></i> Rangées(<?php 
        $sql3 ="SELECT id_ranger from ranger ";
        $query3 = $dbh -> prepare($sql3);
        $query3->execute();
        $results3=$query3->fetchAll(PDO::FETCH_OBJ);
        $regstds=$query3->rowCount();
        ?><?php echo htmlentities($regstds);?>) <a href="ajouter-ranger" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i> Nouveau</a></h4>
    <hr />
        ...
        $update_date=date('d-m-Y à H:i:s',$format_update_date); 
           ...
           ?>                                      
            <tr class="odd gradeX">
              ...
              <td><span class="label label-success"><?php echo htmlentities($result->ranger);?></span></td>
              ...
              <td class="center">
                ...
                <a href="modifier-ranger?rngid=<?php echo htmlentities($result->id_ranger);?>"><i title="Modifier" class="fa fa-edit"></i></a>
                  <a class="delete" href="gestion-ranger?del=<?php echo htmlentities($result->id_ranger);?>" data-id="<?php echo htmlentities($result->id_ranger);?>"><i title="Supprimer" class="fa fa-trash"></i></a>
                  </td>
                </tr>
                <?php $cnt=$cnt+1;}} ?>                                      
              </tbody>
